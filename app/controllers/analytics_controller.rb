class AnalyticsController < ApplicationController
    ...
    def total_views
        user_analytics = find_user_analytics.map{ |a| {project_id: a.project_id, date: a.created_at.to_date } }
    ...
    def pages_visted  
        user_analytics = find_user_analytics.map{ |a| { page: a.page_path, date: a.created_at.to_date } }
        render json: user_analytics.pluck(:page).tally.map{ |e| {page: e[0], count: e[1] }}
    end

    def unique_views
        user_analytics = find_user_analytics .map{ |a| {date: a.created_at.to_date, ip: a.user_ip } }
    ...
    def countries 
        user_analytics = find_user_analytics.map{ |a| {date: a.created_at.to_date, country: a.user_country_name } }
        render json: user_analytics.pluck(:country).tally.map{ |e| {country: e[0], count: e[1] }}
    end

    def device 
        user_analytics = find_user_analytics.map{ |a| { is_mobile: a.is_mobile, date: a.created_at.to_date } }
